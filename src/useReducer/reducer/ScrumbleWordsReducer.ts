...
export type ScrumbleWordsActions =
...
export const getInitialState = (): ScrambleWordState => {
  const suhffledWords = shuffleArray([...GAME_WORDS]);
  return {
    currentWord: suhffledWords[0],
    errorCounter: 0,
    guess: "",
    isGameOver: false,
    maxAllowErrors: 3,
    maxSkips: 3,
    points: 0,
    scrambledWord: scrambleWord(suhffledWords[0]),
    skipCounter: 0,
    words: suhffledWords,
    totalWords: suhffledWords.length,
  };
};
export const ScrumbleWordsReducer = (
  state: ScrambleWordState,
  action: ScrumbleWordsActions,
): ScrambleWordState => {
  switch (action.type) {
    case "SET_GUESS":
      return {
